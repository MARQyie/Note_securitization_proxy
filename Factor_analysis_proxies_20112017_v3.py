...
np.random.seed(1)
for i in range(100):
    df_rand = pd.DataFrame(np.random.normal(size = df_standard.shape))
...
fig.savefig('Figures/Dimension_reduction/FA_parallel_20112017.png')
...
fa_loadings.to_csv('Results/fa_loadings_norotation_20112017.csv')

#--------------------------------------------
# Oblique rotations

# Promax 
fa_pm = FactorAnalyzer(rotation = 'promax', n_factors = 2, method = 'principal')
...
fa_pm_loadings.to_csv('Results/fa_loadings_promax_20112017.csv')

# Oblimin
fa_om = FactorAnalyzer(rotation = 'oblimin', n_factors = 2, method = 'principal')
...
fa_om_loadings.to_csv('Results/fa_loadings_oblimin_20112017.csv')

# Quartimin
fa_qmo = FactorAnalyzer(rotation = 'quartimin', n_factors = 2, method = 'principal')
...
fa_qmo_loadings.to_csv('Results/fa_loadings_quartimin_20112017.csv')
...
writer = pd.ExcelWriter('Results/fa_loadings_all_20112017.xlsx', engine='xlsxwriter')
...
# Rename columns
fa_pm_loadings.columns = pd.MultiIndex.from_product([['Two Factor Model',],['F2','F1']])
fa_pm5_loadings.columns = pd.MultiIndex.from_product([['Three Factor Model',],['F2','F1','F3']])

# Combine 4 and 2 factor models
fa_table = pd.concat([fa_pm_loadings['Two Factor Model'][['F1','F2']],\
...
caption = ('Results Explanatory Factor Analysis: Two and Three Factor Models')
...
# Save
text_fa_table_latex_latex = open('Results/EFA_4factors_20112017.tex', 'w')
