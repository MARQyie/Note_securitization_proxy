...
#RC-R
vars_rcr = '|'.join(['IDRSSD', 'S475', 'S480', 'S485', 'S490', 'S495'])
...
def loadRCR(i):
    ''' Dedicated function for loading RC-R data '''
    if i == 2014:
...
'''We use a similar setup as above and loop over
    all years to load data about securitization
...
file_der = r'All_Reports_{}1231_Derivatives.csv'

# Get variable names
vars_sec1 = ['cert', 'fed_rssd', 'SZLNRES', 'SZLNHEL', 'SZLNCRCD',\
            'SZLAUTO', 'SZLNCON', 'SZLNCI','SZLNOTH', 'ASDRRES',\
            'ASDRHEL', 'ASDRCRCD', 'ASDRAUTO', 'ASDRCONS', 'ASDRCI',\
            'ASDROTH']
vars_sec2 = ['cert', 'fed_rssd', 'SZLNRES', 'SZLNHEL', 'SZLNCRCD',\
            'SZLAUTO', 'SZLNCON', 'SZLNCI','SZLNOTH', 'ASDRRES',\
            'ASDROTH']
vars_der = ['cert', 'fed_rssd', 'ctdergty','ctderben']

#--------------------------------------------
# Set functions

def loadSDISec(i, file, var_list1, var_list2):
    ''' A General function for loading SDI data, no breaks in file
        names'''
    try:
        df_load = pd.read_csv((path_sdi + file).format(i,i), usecols = var_list1)
    except:
        df_load = pd.read_csv((path_sdi + file).format(i,i), usecols = var_list2)   
    df_load['date'] = int('{}'.format(i))  
    
    return(df_load)
    

def loadSDIDer(i, file, var_list):
    ''' A General function for loading SDI data, no breaks in file
        names'''
    
    df_load = pd.read_csv((path_sdi + file).format(i,i), usecols = var_list)
    df_load['date'] = int('{}'.format(i))  
...
# Run functions
if __name__ == '__main__':
    df_sec = pd.concat(Parallel(n_jobs=num_cores)(delayed(loadSDISec)(i, file_sec, vars_sec1, vars_sec2) for i in range(start, end)))
    df_der = pd.concat(Parallel(n_jobs=num_cores)(delayed(loadSDIDer)(i, file_der, vars_der) for i in range(start, end)))
    
# Merge dfs
cols_to_use = ['cert', 'date'] + df_der.columns.difference(df_sec.columns).tolist()
df_sdi = df_sec.merge(df_der[cols_to_use], on = ['cert', 'date'], how = 'left')
...
vars_lf = ['hmprid'] + ['CERT{}'.format(str(year)[2:4]) for year in range(start, end - 2)]
...
# 1) SDI LF
if __name__ == "__main__":
    df_sdilf = pd.concat(Parallel(n_jobs=num_cores)(delayed(mergeSDILF)(year) for year in range(start,end)))
...
# 2) SDILF HMDA
df_hmdasdi = df_sdilf.merge(df_hmda, left_on = ['date','hmprid'],\
                            right_on = ['date','respondent_id'], how = 'left')
    
# 3) HMDASDI CR
df_raw = df_cr.reset_index().merge(df_hmdasdi, left_on = ['date','IDRSSD'],\
                            right_on = ['date','fed_rssd'], how = 'left')
...
# Total Assets VIE
df['cr_ta_vie'] = df.loc[:,['cr_ta_secveh','cr_ta_abcp','cr_ta_vie_other']].sum(axis = 1) \
                  + df_raw.RCONFT14

# Assets sold and securitized with recourse
df['sdi_as_sec'] = df_raw.loc[:,['SZLNRES', 'SZLNHEL', 'SZLNCRCD',\
            'SZLAUTO', 'SZLNCON', 'SZLNCI','SZLNOTH']].sum(axis = 1)

# Assets sold and not securitized with recourse
df['sdi_as_nonsec'] = df_raw.loc[:,['ASDRRES',\
            'ASDRHEL', 'ASDRCRCD', 'ASDRAUTO', 'ASDRCONS', 'ASDRCI',\
            'ASDROTH']].sum(axis = 1)

# Sold protection credit derivatives
df['sdi_cd_sold'] = df_raw.ctdergty

# Purchased protection credit derivatives
df['sdi_cd_purchased'] = df_raw.ctderben
