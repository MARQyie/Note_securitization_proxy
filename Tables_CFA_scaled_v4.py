        ...
        if row['lhs'] in lst_factors:
            if row['rhs'] in lst_factors:
                if row['lhs'] == row['rhs']:
                    ...
                    index_lst.append(['Covariance ($\phi$)', str(factor_dict[row['lhs']]), str(factor_dict[row['rhs']])])
...
def tidyFitInd(data):
    ...
    lst_fi_labels = ['No. Params','DoF','DoF baseline','SRMR', 'RMSEA',\
                     'RMSEA lower bound', 'RMSEA upper bound',\
                     'RNI', '$\chi^2$','p-val $\chi^2$','Yuan--Bentler $\chi^2$',\
                     'p-val Yuan--Bentler $\chi^2$','$\chi^2$ scaling factor',\
                     '$\chi^2$ baseline', 'Yuan--Bentler $\chi^2$ baseline',\
                     'CFI', 'IFI', 'NFI','TLI',\
                     'AIC','BIC','GFI','AGFI']
...
# fit indices 
df_fi_onesec = pd.read_csv('Results/CFA_fitmeasures_onesec_scaled.csv', index_col = 0)

df_fi_efa = pd.read_csv('Results/CFA_fitmeasures_efa_scaled.csv', index_col = 0)
df_fi_theory = pd.read_csv('Results/CFA_fitmeasures_theory_scaled.csv', index_col = 0)
df_fi_combined = pd.read_csv('Results/CFA_fitmeasures_combined_scaled.csv', index_col = 0)

# Parameter estimates
df_params_onesec = pd.read_csv('Results/CFA_params_onesec_scaled.csv', index_col = 0)

df_params_efa = pd.read_csv('Results/CFA_params_efa_scaled.csv', index_col = 0)
df_params_theory = pd.read_csv('Results/CFA_params_theory_scaled.csv', index_col = 0)
df_params_combined = pd.read_csv('Results/CFA_params_combined_scaled.csv', index_col = 0)
...
## To latex
### Prelims
dict_options = {'column_format':'p{4.75cm}' + 'p{1.5cm}' * df_fi_onesec_tidy.shape[1],
                'caption':('Fit Indices: One Securitization Factor Model (Scaled)'),
                'label':'tab:cfa_fit_onesec_scaled',
                'position':'th'}
notes = '\\multicolumn{2}{p{6cm}}{\\textit{Notes.} Fit indices of the model with one securitization factor scaled by total assets.} \n'
...
text_latex_fi_onesec = open('Results/CFA_fi_onesec_scaled.tex', 'w')
...
## To latex
### Prelims
dict_options = {'column_format':'p{4.75cm}' + 'p{1.75cm}' * df_fi_2f_tidy.shape[1],
                'caption':('Fit Indices: Two-factor Models (Scaled)'),
                'label':'tab:cfa_fit_twofactor_models_scaled',
                'position':'th'}
notes = '\\multicolumn{4}{p{10.75cm}}{\\textit{Notes.} Fit indices of the two-factor models scaled by total assets. The first column contains the fit indices for the model based on the exploratory factor analysis (EFA), and the second column the model based on Table \\ref{tab:possible_proxies}. The last column presents the fit indices for the model where we combine exploratory factor analysis and Table \\ref{tab:possible_proxies}.} \n'
string_size = '\\scriptsize\n'

### Get latex table and save
latex_fi_2f = table2Latex(df_fi_2f_tidy,dict_options,notes,string_size)
text_latex_latex_fi_2f = open('Results/CFA_fi_2f_scaled.tex', 'w')
...
dict_options = {'column_format':'p{4.75cm}p{4.75cm}' + 'p{1cm}' * df_params_onesec_tidy.shape[1],
                'caption':('Factor Loadings, Variances and Covariances: One Securitization Factor Model (Scaled)'),
                'label':'tab:cfa_table_onesec_scaled',
                'position':'th'}
notes = '\\multicolumn{6}{p{16cm}}{\\textit{Notes.} Factor loadings, factor variances and covariances, and unique variances and covariances of the factor model with one factor scaled by total assets. The last column presents the completely standardized estimates, where the factor variances are fixed to one and all other parameters are standardized.} \n'
string_size = '\\scriptsize\n'

### Get latex table and save
latex_params_onesec = table2Latex(df_params_onesec_tidy,dict_options,notes,string_size)
text_latex_params_onesec = open('Results/CFA_params_onesec_scaled.tex', 'w')
...
dict_options = {'column_format':'p{4.75cm}p{4.75cm}' + 'p{1cm}' * df_params_efa_tidy.shape[1],
                'caption':('Factor Loadings, Variances and Covariances: Two-factor EFA Model (Scaled)'),
                'label':'tab:cfa_table_2fefa_scaled',
                'position':'th'}
notes = '\\multicolumn{6}{p{16cm}}{\\textit{Notes.} Factor loadings, factor variances and covariances, and unique variances and covariances of the two-factor model based on the exploratory factor analysis (EFA) scaled by total assets. The last column presents the completely standardized estimates, where the factor variances are fixed to one and all other parameters are standardized.} \n'
string_size = '\\scriptsize\n'

### Get latex table and save
latex_params_efa = table2Latex(df_params_efa_tidy,dict_options,notes,string_size)
text_latex_params_onesec = open('Results/CFA_params_efa_scaled.tex', 'w')
...
dict_options = {'column_format':'p{4.75cm}p{4.75cm}' + 'p{1cm}' * df_params_theory_tidy.shape[1],
                'caption':('Factor Loadings, Variances and Covariances: Two-factor Theory Model (Scaled)'),
                'label':'tab:cfa_table_2ftheory_scaled',
                'position':'th'}
notes = '\\multicolumn{6}{p{16cm}}{\\textit{Notes.} Factor loadings, factor variances and covariances, and unique variances and covariances of the two-factor model based on Table \\ref{tab:possible_proxies} scaled by total assets. The last column presents the completely standardized estimates, where the factor variances are fixed to one and all other parameters are standardized.} \n'
string_size = '\\scriptsize\n'

### Get latex table and save
latex_params_theory = table2Latex(df_params_theory_tidy,dict_options,notes,string_size)
text_latex_params_theory = open('Results/CFA_params_theory_scaled.tex', 'w')
...
dict_options = {'column_format':'p{4.75cm}p{4.75cm}' + 'p{1cm}' * df_params_combined_tidy.shape[1],
                'caption':('Factor Loadings, Variances and Covariances: Two-factor Combined Model (Scaled)'),
                'label':'tab:cfa_table_2fcombined_scaled',
                'position':'th'}
notes = '\\multicolumn{6}{p{16cm}}{\\textit{Notes.} Factor loadings, factor variances and covariances, and unique variances and covariances of the two-factor model based on the exploratory factor analysis (EFA) and Table \\ref{tab:possible_proxies} scaled by total assets. The last column presents the completely standardized estimates, where the factor variances are fixed to one and all other parameters are standardized.} \n'
string_size = '\\scriptsize\n'

### Get latex table and save
latex_params_combined = table2Latex(df_params_combined_tidy,dict_options,notes,string_size)
text_latex_params_combined = open('Results/CFA_params_combined_scaled.tex', 'w')
